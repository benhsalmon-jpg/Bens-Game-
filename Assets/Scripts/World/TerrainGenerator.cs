...
public class TerrainGenerator : MonoBehaviour
{
    ...
    [Header("Vegetation / GPU Instancing")]
    [Tooltip("Master toggle for GPU grass.")]
    [SerializeField] private bool enableGrass = true;
    [Tooltip("Assign ANY mesh here (quad, grass blade FBX, bush, etc.).")]
    [SerializeField] private Mesh vegetationMesh;
    [Tooltip("Material MUST have Enable GPU Instancing. Prefer Custom/GrassInstanced or URP/Unlit.")]
    [SerializeField] private Material vegetationMaterial;
    [Tooltip("World spacing between grass cells. Smaller = denser.")]
    [SerializeField] private float grassCellSize = 1.25f;
    [Range(0.05f, 2f)]
    [Tooltip("Multiplies every biome grass density.")]
    [SerializeField] private float grassDensityMultiplier = 1f;
    [SerializeField] private bool createDefaultGrassIfMissing = true;
    [SerializeField] private bool forceEnableInstancingOnMaterials = true;
    [Tooltip("Submesh index for assigned meshes (usually 0).")]
    [SerializeField] private int grassSubmeshIndex = 0;
    [Tooltip("If true, ignores biome density=0 and still plants grass.")]
    [SerializeField] private bool forceGrassWhenEnabled = true;
    ...
    private int lastGeneratedGrassCount;
    private bool vegetationDirty = true;
    private int emptyVegetationRebuildAttempts;
    ...
    private void Start()
    {
        ...
        vegetationSystem = new VegetationSystem(seed, grassCellSize, 45f, grassDensityMultiplier);
    ...
    private void OnGUI()
    {
        // Always show grass draw count while grass is enabled (easy GPU instancing debug).
        if (enableGrass)
        {
            GUI.Box(new Rect(12, 12, 360, 92), "GPU Grass Debug");
            GUI.Label(new Rect(24, 36, 340, 64),
                $"Generated: {lastGeneratedGrassCount} | Drawn: {lastDrawnGrassCount}\n" +
                $"Mesh: {(vegetationMesh != null ? vegetationMesh.name : "NONE")} | " +
                $"Mat: {(vegetationMaterial != null ? vegetationMaterial.name : "NONE")}\n" +
                $"Chunks: {activeChunks.Count} | Enable={enableGrass}");
        }

        if (!enableDeterminismProbe || string.IsNullOrEmpty(lastDeterminismReport))
            return;

        GUI.Box(new Rect(12, 100, 440, 150), "Determinism Probe");
        GUI.Label(new Rect(24, 124, 416, 120), lastDeterminismReport);
    }
    ...
    private IEnumerator GenerateChunkStaged(Vector2Int chunkCoord)
    {
        ...
        // Always rebuild grass after a chunk finishes — player may not have moved enough
        // for UpdateVegetationDistanceFade to run again.
        ...
        for (int x = 0; x < chunkSize && spawned < maxObjectsPerChunk; x += stride)
        {
            for (int z = 0; z < chunkSize && spawned < maxObjectsPerChunk; z += stride)
            {
                ...
                for (int i = 0; i < biome.objects.Length; i++)
                {
                    ...
                    // Keep height sample inside this chunk so objects don't float from out-of-range bilinear reads.
                    float placedLocalX = Mathf.Clamp(worldX - coord.x * chunkSize, 0f, chunkSize);
                    float placedLocalZ = Mathf.Clamp(worldZ - coord.y * chunkSize, 0f, chunkSize);
                    worldX = coord.x * chunkSize + placedLocalX;
                    worldZ = coord.y * chunkSize + placedLocalZ;
                    float y = data.SampleHeightBilinear(placedLocalX, placedLocalZ);
                    Vector3 placeNormal = data.SampleNormalBilinear(placedLocalX, placedLocalZ);
                    ...
                    PositionObjectOnTerrain(instance, y, obj.yOffset, placeNormal, obj.alignToTerrainNormal, obj.normalAlignStrength);
    ...
    /// <summary>
    /// Places the object's lowest bound exactly on the terrain surface (plus yOffset),
    /// regardless of whether the prefab pivot is at the center, base, or elsewhere.
    /// </summary>
    private void PositionObjectOnTerrain(
        GameObject objectInstance,
        float terrainElevation,
        float yOffset = 0f,
        Vector3 terrainNormal = default,
        bool alignToNormal = false,
        float normalAlignStrength = 0.85f)
    {
        if (objectInstance == null)
            return;

        if (alignToNormal && terrainNormal.sqrMagnitude > 0.0001f)
        {
            Vector3 up = Vector3.Normalize(Vector3.Lerp(Vector3.up, terrainNormal.normalized, Mathf.Clamp01(normalAlignStrength)));
            Vector3 yawForward = objectInstance.transform.rotation * Vector3.forward;
            Vector3 projected = Vector3.ProjectOnPlane(yawForward, up);
            if (projected.sqrMagnitude < 0.0001f)
                projected = Vector3.ProjectOnPlane(objectInstance.transform.rotation * Vector3.right, up);
            if (projected.sqrMagnitude > 0.0001f)
                objectInstance.transform.rotation = Quaternion.LookRotation(projected.normalized, up);
            else
                objectInstance.transform.rotation = Quaternion.FromToRotation(Vector3.up, up) * objectInstance.transform.rotation;
        }

        // Bounds are world-space; sync after rotation/position so grounding uses the final pose.
        Physics.SyncTransforms();

        if (TryGetObjectBounds(objectInstance, out Bounds bounds))
        {
            Vector3 pos = objectInstance.transform.position;
            float targetBottom = terrainElevation + yOffset;
            pos.y += targetBottom - bounds.min.y;
    ...
    private static bool TryGetObjectBounds(GameObject objectInstance, out Bounds bounds)
    {
        bounds = default;
    ...
    private void UpdateVegetationDistanceFade()
    {
        ...
        Vector3 playerPos = playerTransform.position;
        bool movedEnough =
            (playerPos - lastVegetationPlayerPos).sqrMagnitude >=
            VegetationRefreshMoveThreshold * VegetationRefreshMoveThreshold;

        if (movedEnough)
            emptyVegetationRebuildAttempts = 0;

        // Rebuild when the player moves, a chunk just finished, or nearby grass is still empty
        // (capped so we don't spin every frame if planting truly fails).
        bool needsEmptyRebuild =
            vegetationDirty ||
            (emptyVegetationRebuildAttempts < 4 && HasEmptyNearbyVegetation(playerPos));

        if (!movedEnough && !needsEmptyRebuild)
            return;

        if (!movedEnough && needsEmptyRebuild)
            emptyVegetationRebuildAttempts++;

        lastVegetationPlayerPos = playerPos;
        vegetationDirty = false;
        RebuildAllVegetation(playerPos);
    }

    private bool HasEmptyNearbyVegetation(Vector3 playerPos)
    {
        Vector2Int playerChunk = GetChunkCoordinates(playerPos);
        ...
        foreach (var kvp in activeChunks)
        {
            ...
            if (ChunkDistanceSq(playerChunk, kvp.Key) > vegLoadSq) continue;

            int count = chunk.Data.GrassInstanceCount;
            if (count <= 0)
                return true;
            if (chunk.GrassMatrices == null || chunk.GrassMatrices.Count == 0)
                return true;
        }
    ...
    private void RebuildAllVegetation(Vector3 playerPos)
    {
        Vector2Int playerChunk = GetChunkCoordinates(playerPos);
        float vegLoadSq = vegetationLoadDistance * vegetationLoadDistance;
        float vegUnloadSq = vegetationUnloadDistance * vegetationUnloadDistance;
        lastGeneratedGrassCount = 0;

        vegetationSystem.forceGrassWhenEnabled = forceGrassWhenEnabled;
        ...
        foreach (var kvp in activeChunks)
        {
            ...
            if (distSq > vegUnloadSq)
            {
                ...
                if (chunk.Data != null)
                    chunk.Data.GrassInstanceCount = 0;
                continue;
            }
            ...
            vegetationSystem.GenerateGrass(
                chunk.Data,
                playerPos,
                vegetationLoadDistance,
                grassScratch,
                matrices,
                biomes);
            chunk.SetGrassMatrices(matrices);
            lastGeneratedGrassCount += chunk.Data.GrassInstanceCount;
    ...
    private void DrawVegetation()
    {
        ...
        if (vegetationMesh == null || vegetationMaterial == null)
            EnsureDefaultGrassPrototypes();
        ...
        foreach (var kvp in activeChunks)
        {
            ...
            foreach (var entry in matrices)
            {
                ...
                Material mat = vegetationMaterial;
                ...
                if (biomes != null && biomeIndex >= 0 && biomeIndex < biomes.Length)
                {
                    ...
                    GrassType[] types = biome?.vegetation?.grassTypes;
                    if (types != null && grassTypeIndex >= 0 && grassTypeIndex < types.Length)
                    {
                        GrassType grass = types[grassTypeIndex];
                        if (grass != null)
                        {
                            if (grass.mesh != null) mesh = grass.mesh;
                            if (grass.material != null) mat = grass.material;
                        }
                    }
                }
                ...
                PrepareGrassMaterial(mat);

                int submesh = Mathf.Clamp(grassSubmeshIndex, 0, Mathf.Max(0, mesh.subMeshCount - 1));
                int offset = 0;
    ...
    {
        // Resolve global vegetation fallbacks (separate from elevation grassMaterial).
        if (vegetationMesh == null && createDefaultGrassIfMissing)
            vegetationMesh = VegetationRendererUtil.CreateDefaultGrassBladeMesh();

        if (vegetationMaterial == null && createDefaultGrassIfMissing)
            vegetationMaterial = VegetationRendererUtil.CreateDefaultGrassMaterial(new Color(0.25f, 0.85f, 0.2f, 1f));

        if (vegetationMaterial != null)
            PrepareGrassMaterial(vegetationMaterial);

        defaultGrassMesh = vegetationMesh;
        defaultGrassMaterial = vegetationMaterial;
        ...
        for (int i = 0; i < biomes.Length; i++)
        {
            ...
            // Ensure usable density so grass actually plants.
            if (forceGrassWhenEnabled && biome.vegetation.density < 0.05f)
                biome.vegetation.density = 0.85f;
            if (biome.vegetation.farDistance < biome.vegetation.nearDistance + 1f)
                biome.vegetation.farDistance = Mathf.Max(vegetationLoadDistance, biome.vegetation.nearDistance + 40f);
            if (biome.vegetation.slopeLimit < 5f)
                biome.vegetation.slopeLimit = 45f;

            if (biome.vegetation.grassTypes == null || biome.vegetation.grassTypes.Length == 0)
            {
                if (!createDefaultGrassIfMissing && vegetationMesh == null)
                    continue;
                ...
                biome.vegetation.grassTypes = new[]
                {
                    new GrassType
                    {
                        ...
                        density = 1f,
                        minScale = biome.vegetation.minScale > 0f ? biome.vegetation.minScale : 0.85f,
                        maxScale = biome.vegetation.maxScale > 0f ? biome.vegetation.maxScale : 1.35f,
                        randomYRotation = 360f,
                        maxSlope = Mathf.Max(40f, biome.vegetation.slopeLimit)
                    }
                };
            }
            ...
            for (int g = 0; g < biome.vegetation.grassTypes.Length; g++)
            {
                ...
                if (gt == null)
                    biome.vegetation.grassTypes[g] = gt = new GrassType();

                if (gt.mesh == null)
                    gt.mesh = vegetationMesh;
                if (gt.material == null)
                    gt.material = vegetationMaterial;
                if (forceGrassWhenEnabled && gt.density < 0.05f)
                    gt.density = 1f;
                if (gt.maxSlope < 5f)
                    gt.maxSlope = 45f;

                if (gt.material != null)
                    PrepareGrassMaterial(gt.material);
            }
        }

        if (showDebugLogs || enableGrass)
        {
            int configured = 0;
            for (int i = 0; i < biomes.Length; i++)
            {
                if (biomes[i]?.vegetation?.grassTypes == null) continue;
                for (int g = 0; g < biomes[i].vegetation.grassTypes.Length; g++)
                {
                    GrassType gt = biomes[i].vegetation.grassTypes[g];
                    if (gt != null && gt.mesh != null && gt.material != null)
                        configured++;
                }
            }

            Debug.Log(
                $"[TerrainGenerator] Grass ready. Mesh={(vegetationMesh != null ? vegetationMesh.name : "null")} " +
                $"Mat={(vegetationMaterial != null ? vegetationMaterial.name : "null")} Types={configured} " +
                $"Cell={grassCellSize} DensityMul={grassDensityMultiplier}",
                this);
        }
    }

    private void PrepareGrassMaterial(Material mat)
    {
        if (mat == null)
            return;

        if (forceEnableInstancingOnMaterials)
            mat.enableInstancing = true;

        if (mat.HasProperty("_Cull"))
            mat.SetFloat("_Cull", 0f);
        if (mat.HasProperty("_Color") && mat.GetColor("_Color").a < 0.01f)
            mat.SetColor("_Color", new Color(0.25f, 0.85f, 0.2f, 1f));
        if (mat.HasProperty("_BaseColor") && mat.GetColor("_BaseColor").a < 0.01f)
            mat.SetColor("_BaseColor", new Color(0.25f, 0.85f, 0.2f, 1f));
    }
...
{
    [Tooltip("Assign ANY mesh (grass blade, bush, rock detail, etc.). Leave empty to use TerrainGenerator Vegetation Mesh.")]
    public Mesh mesh;
    [Tooltip("Must have Enable GPU Instancing. Leave empty to use TerrainGenerator Vegetation Material.")]
...
public class BiomeObject
{
    ...
    [Tooltip("Extra vertical nudge after grounding. Negative buries slightly into terrain.")]
    public float yOffset;
    [Tooltip("Tilt the prefab to follow the terrain normal so roots sit flush on slopes.")]
    public bool alignToTerrainNormal = true;
    [Range(0f, 1f)]
    [Tooltip("0 = upright, 1 = fully match terrain normal.")]
    public float normalAlignStrength = 0.85f;
...
public class VegetationSystem
{
    ...
    /// <summary>When true, biome density=0 / empty grassTypes still plant using defaults.</summary>
    public bool forceGrassWhenEnabled = true;
        ...
        List<VegetationInstance> output, Dictionary<int, List<Matrix4x4>> matricesByGrassKey)
    {
        ...
        BiomeVegetationSettings veg = biome.vegetation ?? new BiomeVegetationSettings();
        GrassType[] types = veg.grassTypes;
        bool hasTypes = types != null && types.Length > 0;

        if (!hasTypes && !forceGrassWhenEnabled)
            return;

        float slopeLimit = veg.slopeLimit > 1f ? veg.slopeLimit : 45f;
        if (height < veg.minElevation || height > veg.maxElevation || slope > slopeLimit)
            return;

        float near = Mathf.Max(1f, veg.nearDistance > 0f ? veg.nearDistance : 40f);
        float far = Mathf.Max(near + 0.01f, veg.farDistance > near ? veg.farDistance : near + 80f);
        float distanceFade = 1f - Mathf.SmoothStep(near, far, Mathf.Sqrt(distSq));
        if (distanceFade <= 0.001f)
            return;

        float vegDensity = veg.density;
        if (forceGrassWhenEnabled && vegDensity < 0.05f)
            vegDensity = 0.85f;

        float density = Mathf.Clamp01(vegDensity) * Mathf.Clamp01(biomeWeight) * distanceFade * Mathf.Max(0.05f, densityMultiplier);
        if (roll > density)
            return;

        int typeIndex = 0;
        GrassType grass = null;
        if (hasTypes)
        {
            typeIndex = SelectGrassType(types, hash);
            if (typeIndex < 0)
            {
                if (!forceGrassWhenEnabled)
                    return;
                typeIndex = 0;
            }
            else
            {
                grass = types[typeIndex];
            }
        }
        ...
        if (grass != null)
        {
            ...
            if (forceGrassWhenEnabled && typeDensity < 0.05f)
                typeDensity = 1f;
            ...
            if (height < grass.minElevation || height > grass.maxElevation)
                return;
            float grassSlope = grass.maxSlope > 1f ? grass.maxSlope : 45f;
            if (slope > grassSlope)
                return;
        }

        float minScale = grass != null && grass.minScale > 0f
            ? grass.minScale
            : (veg.minScale > 0f ? veg.minScale : 0.85f);
        float maxScale = grass != null && grass.maxScale > 0f
            ? grass.maxScale
            : (veg.maxScale > 0f ? veg.maxScale : 1.35f);
        if (maxScale < minScale) maxScale = minScale;
        float scale = WorldSeed.ToFloatRange(WorldSeed.Mix(hash, 41), minScale, maxScale);
        float yawMax = grass != null && grass.randomYRotation > 0f
            ? grass.randomYRotation
            : (veg.randomRotation > 0f ? veg.randomRotation : 360f);
